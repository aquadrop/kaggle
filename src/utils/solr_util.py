...
def compose_fq(mapper, option_fields=['price']):
    ...
    if len(must_mapper) > 0:
        must = " AND ".join(["{}:{}".format(key, value)
                             for key, value in must_mapper.items() if value and value != '*:*'])
        fq = "*:* AND " + must
...
def compose_q(mapper, option_fields=['price']):
    option_mapper = dict()
    for of in option_fields:
        if of in mapper:
            option_mapper[of] = mapper[of]
    ...
    fq = '*:*'
    if len(must_mapper) > 0:
        must = " AND ".join(["{}:{}".format(key, value) for key, value in must_mapper.items() if value and value != '*:*'])
        fq = must
    if len(option_mapper) > 0:
        fq = fq + ' OR ' + " OR ".join(["{}:{}".format(key, value) for key, value in option_mapper.items()])
...
def solr_qa(core, query, solr=solr, field=None, cls=''):
    if not cls:
        if not field:
...
class SimpleSolr(object):
    def __init__(self, core, url='http://localhost:8999/solr'):
        self.core = core
        self.solr = SolrClient(url)

    def fq(self, q_mapper={}, fq_mapper={}, retrieve_keys=[], rows=10):

        # special treatmeat
        if 'entities' in fq_mapper and fq_mapper['entities'] and fq_mapper['entities'] != '*:*':
            fq_mapper['entities'] = "\"" + fq_mapper['entities'] + "\""

        params = {
            'q': compose_q(q_mapper),
            'fq': compose_fq(fq_mapper),
            'rows':rows
        }

        if len(retrieve_keys) > 0:
            params['fl'] = ','.join(retrieve_keys)
    ...
    def solr_facet(self,
        ...
        if not is_range:
            ...
            params['fq'] = compose_fq(mappers)
            res = solr.query(self.core, params)
            facets = res.get_facet_keys_as_list(facet_field)
            docs = res.docs
            return facets, len(facets), docs
        else:
    ...
    @staticmethod
    def concat_solr_list(array_list):
...
if __name__ == "__main__":
    ...
    condition_mapper = {"store_id":"吴江新华书店", "intent":"(scanOkOk)"}
    q_mapper = {"question":"扫好了"}
    simple_solr = SimpleSolr('automata')
    print(simple_solr.fq(q_mapper, condition_mapper, retrieve_keys=['question']))